...
with DAG ('branch_sql_dag',default_args = default_args, schedule_interval = '@daily', catchup = False) as dag:
    ...
    create_table = CustomPostgresOperator(
        ...
        sql = "sql/create_partner_table.sql"
    )

    insert_table = CustomPostgresOperator(
        task_id = 'insert_table',
        postgres_conn_id = 'postgres',
        sql = "sql/insert_into_partners.sql"
    )

    check_op = BranchSQLOperator(
        task_id = 'check_op',
        conn_id = 'postgres',
        sql = "select count(distinct partner_status)-1 from partners", #-> Returns 0 if all cols set to True, else returns 1
        # check if all columns are set to True
        follow_task_ids_if_true = ['inaccurate'],
        follow_task_ids_if_false = ['accurate']
    )
    ...
    create_table >> insert_table >> check_op >> [accurate, inaccurate]
